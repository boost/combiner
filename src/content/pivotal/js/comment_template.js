...
const extractDropdown = () => {
  ...
  $attachment
    .parents('span[data-aid="CommentEditToolbar__container"]')
    .toggle();

  // get the dropdown
  $attachment.trigger("click");
  const $dropdown = $attachment
    .parent("a")
    .find('[data-aid="attachmentDropdownButton--dropDownMenuWrapper"]')
    .clone();
  $attachment.trigger("click");
...
const insertTemplateButton = ($spanButtons) => {
  const $templateDiv = $spanButtons
    .find("> div")
    .first()
    .clone()
    .appendTo($spanButtons);
  $templateDiv.find("button").attr("data-aid", "template");
  $templateDiv
    .find('[data-aid="template"] span')
    .attr(
      "style",
      'background: url("//assets.pivotaltracker.com/next/assets/next/b202db4f-story-templates.svg") center center no-repeat;'
    );
  $templateDiv
    .find('[data-aid="template"] span')
    .attr("title", "Insert the ready for acceptance template in the comment");
...
const initCommentTemplate = () => {
  ...
  setupDropdown(
    $dropdown,
    $spanButtons.parents('[data-aid="comment-new"]').find("textarea")
  );

  $spanButtons
    .parents('[data-aid="StoryDetailsEdit"]')
    .find('textarea[aria-label="story title"]')
    .focus();
};
