...
let getIterationStories = async (client, iteration, project = null) => {
  const cIteration = await client.iteration(
...
let getUserOwnedStories = async (client, project) => {
  ...
  stories = stories.concat(
    await client.stories(project.id, { with_state: "finished" })
  );
...
let enrichStory = async (client, story, options) => {
  ...
  if (options.includes("requester")) {
    ...
    story.requester = memberships.find(
      (u) => u.person.id === story.requested_by_id
    );
  }
...
let sendDetails = async (client, story, tab, title, description) => {
  ...
  return browser.tabs.sendMessage(tab.id, {
    title: evalTemplate(enrichedStory, title),
    description: evalTemplate(enrichedStory, description),
  });
};

let sendR4ADetails = async (client, story, tab) => {
  const titleTemplate = await getTemplates(
    client,
    "r4a-title",
    story.project_id,
    browser.i18n.getMessage("readyForAcceptanceTitleTemplate")
  );
  const descriptionTemplate = await getTemplates(
    client,
    "r4a-description",
    story.project_id,
    browser.i18n.getMessage("readyForAcceptanceDescriptionTemplate")
  );

  return await sendDetails(
    client,
    story,
    tab,
...
let sendPrDetails = async (client, story, tab) => {
  const titleTemplate = await getTemplates(
    ...
    "pr-title",
    story.project_id,
    browser.i18n.getMessage("prTitleTemplate")
  );
  const descriptionTemplate = await getTemplates(
    client,
    "pr-description",
    story.project_id,
    browser.i18n.getMessage("prDescriptionTemplate")
  );
