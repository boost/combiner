...
class Header extends Component {
  ...
  getTitle() {
    switch (this.props.active) {
      ...
      case "pivotal":
        return `Iteration ${
          this.state.iteration ? this.state.iteration.number : ""
        }`;
      case "utils":
        return "Utils";
      case "settings":
  ...
  handlePopOut() {
    ...
    if (
      href.indexOf("uilocation=tab") > -1 ||
      href.indexOf("uilocation=popout") > -1
    ) {
      replace = "uilocation=popout";
    }
    href = href
      .replace("uilocation=popup", replace)
  ...
  render() {
    return (
      <header className="grid-x">
        ...
        <h1 className="cell auto">{this.getTitle(this.props.active)}</h1>
      </header>
    );
  }
}
